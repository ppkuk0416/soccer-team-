        ...
        <div className="bg-white rounded-2xl shadow-sm p-5">
          ...
          {players.filter((p) => p.status === 'confirmed').length === 0 ? (
            ...
            <form onSubmit={handleSubmitRequest} className="space-y-3">
              <select
                className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-400"
                    ...
                    <div className="flex gap-1">
                      {tiers.map((t) => (
                        <button key={t} type="button" onClick={() => setReqTier(t)}
                          className={`text-xs px-2 py-1.5 rounded-xl font-bold flex-1 transition ${
                            reqTier === t ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'
                          }`}>
                          {TIER_LABELS[t]}
                        </button>
                      ))}
              ...
              <input
                className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-400 placeholder:text-gray-400"
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5">
          ...
          <div className="space-y-0">
            {resolvedRequests.map((req) => (
              <div key={req.id} className="flex items-center justify-between py-2.5 border-b border-gray-100 last:border-0">
                ...
                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                  req.status === 'approved' ? 'bg-green-50 text-green-600' : 'bg-gray-100 text-gray-400'
                }`}>
