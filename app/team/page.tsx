    ...
    return (
      <div className="space-y-4">
        <div className="bg-white rounded-2xl shadow-sm p-6">
          ...
          <p className="text-sm text-stone-400 mb-5">팀을 만들면 초대코드가 발급됩니다</p>
          <form onSubmit={handleSaveTeam} className="space-y-3">
            <input
              className="w-full bg-stone-50 border-0 rounded-xl px-4 py-3.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-400 placeholder:text-stone-400"
              value={teamName} onChange={(e) => setTeamName(e.target.value)}
              placeholder="팀 이름 (예: 화요일 FC)" required />
            <textarea
              className="w-full bg-stone-50 border-0 rounded-xl px-4 py-3.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-400 resize-none h-20 placeholder:text-stone-400"
            ...
            <div className="flex gap-2 pt-1">
              ...
              {team && (
                <button type="button" onClick={() => setEditingTeam(false)}
                  className="px-5 bg-stone-100 text-gray-600 rounded-xl text-sm font-semibold transition hover:bg-stone-200">
                  취소
                </button>
              )}
            </div>
    ...
    <div className="space-y-3">
      ...
      <div className="bg-stone-900 rounded-3xl p-5 text-white relative overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-5 relative">
          <div>
            <p className="text-stone-500 text-[10px] font-bold uppercase tracking-widest mb-1.5">MY CLUB</p>
            <h1 className="text-2xl font-black leading-tight">{team.name}</h1>
            {team.description && (
              <p className="text-stone-400 text-sm mt-1 leading-relaxed">{team.description}</p>
            )}
          </div>
          {role === 'admin' && (
            <button
              onClick={() => { setTeamName(team.name); setTeamDesc(team.description ?? ''); setEditingTeam(true); }}
              className="shrink-0 text-stone-500 hover:text-white text-xs border border-stone-700 hover:border-stone-500 rounded-lg px-2.5 py-1.5 transition ml-3">
              수정
            </button>
          )}
        </div>
        ...
        {/* Record row */}
        <div className="flex items-center gap-0 border-t border-stone-800 pt-4 relative">
          <div className="flex-1 text-center">
            <div className="text-2xl font-black text-green-400">{wins}</div>
            <div className="text-stone-600 text-[10px] font-bold mt-0.5">승</div>
          </div>
          <div className="w-px h-8 bg-stone-800" />
          <div className="flex-1 text-center">
            <div className="text-2xl font-black text-stone-400">{draws}</div>
            <div className="text-stone-600 text-[10px] font-bold mt-0.5">무</div>
          </div>
          <div className="w-px h-8 bg-stone-800" />
          <div className="flex-1 text-center">
            <div className="text-2xl font-black text-red-400">{losses}</div>
            <div className="text-stone-600 text-[10px] font-bold mt-0.5">패</div>
          </div>
          <div className="w-px h-8 bg-stone-800" />
          <div className="flex-1 text-center">
            <div className="text-2xl font-black text-white">{players.length}</div>
            <div className="text-stone-600 text-[10px] font-bold mt-0.5">선수</div>
          </div>
          <div className="w-px h-8 bg-stone-800" />
          <div className="flex-1 text-center">
            <div className="text-2xl font-black text-white">{avgScore || '—'}</div>
            <div className="text-stone-600 text-[10px] font-bold mt-0.5">평균</div>
          </div>
        </div>
        ...
        {teamInviteCode && (
          <button onClick={copyInviteCode}
            className="mt-4 w-full flex items-center justify-between bg-stone-800 hover:bg-stone-700 rounded-2xl px-4 py-3 transition relative">
            <div className="text-left">
              <p className="text-stone-500 text-[10px] font-bold uppercase tracking-widest">초대 코드</p>
              <p className="text-white font-black text-lg tracking-[0.3em] mt-0.5">{teamInviteCode}</p>
            </div>
            <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${copiedCode ? 'bg-green-600 text-white' : 'bg-stone-700 text-stone-400'}`}>
              {copiedCode ? '복사됨' : '복사'}
            </span>
          </button>
        )}
      ...
      <div className="grid grid-cols-3 gap-2">
        <Link href="/attend">
          <div className="bg-white rounded-2xl shadow-sm p-3.5 text-center hover:shadow-md transition">
            ...
            <div className="text-[10px] text-stone-400 font-semibold mt-0.5">열린 일정</div>
          </div>
        </Link>
        <Link href="/dues">
          <div className="bg-white rounded-2xl shadow-sm p-3.5 text-center hover:shadow-md transition">
            ...
            <div className="text-[10px] text-stone-400 font-semibold mt-0.5">이번달 미납</div>
          </div>
        </Link>
        <Link href="/stats">
          <div className="bg-white rounded-2xl shadow-sm p-3.5 text-center hover:shadow-md transition">
            <div className="text-xl font-black text-gray-900">{matchRecords.length}</div>
            <div className="text-[10px] text-stone-400 font-semibold mt-0.5">총 경기</div>
          </div>
        </Link>
      </div>
      ...
      <div>
        <div className="flex justify-between items-center mb-3">
          <h2 className="font-black text-gray-900 text-base">
            선수 명단
            <span className="text-stone-400 font-normal text-sm ml-1.5">({players.length}명)</span>
          </h2>
          <div className="flex gap-2">
            {role === 'admin' && (
              <Link href="/admin"
                className="text-xs text-stone-400 hover:text-gray-700 font-semibold px-3 py-1.5 rounded-lg bg-white shadow-sm hover:shadow transition">
                평가 관리
              </Link>
            )}
            {role === 'admin' && (
        ...
        {players.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm text-center py-16">
            ...
            <p className="text-stone-400 text-sm mt-1">첫 선수를 추가해보세요</p>
            {role === 'admin' && (
              <button onClick={() => setShowAddPlayer(true)}
              ...
              return (
                <Link key={p.id} href={`/player/${p.id}`}>
                  <div className="bg-white rounded-2xl shadow-sm px-4 py-3.5 flex items-center gap-3 active:scale-[0.99] transition-all cursor-pointer">
                    <span className="text-[11px] font-bold text-stone-300 w-4 text-center shrink-0">{i + 1}</span>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {p.position && (
                          <span className="text-[10px] text-stone-400 font-semibold bg-stone-100 px-1.5 py-0.5 rounded-md">
                            {p.position}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {attendCount > 0 && (
                          <span className="text-[10px] text-stone-400">출석 {attendCount}회</span>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className={`text-2xl font-black ${p.status === 'measuring' ? 'text-stone-200' : 'text-gray-800'}`}>
