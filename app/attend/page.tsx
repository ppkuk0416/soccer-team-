...
const STATUS_CONFIG: { status: VoteStatus; label: string; base: string; active: string }[] = [
  { status: 'attending', label: '참석', base: 'text-gray-500 bg-gray-100', active: 'text-green-700 bg-green-100 font-bold' },
  { status: 'maybe',    label: '미정',  base: 'text-gray-500 bg-gray-100', active: 'text-yellow-700 bg-yellow-100 font-bold' },
  { status: 'absent',   label: '불참', base: 'text-gray-500 bg-gray-100', active: 'text-red-600 bg-red-100 font-bold' },
];
...
function QuarterGrid({ totalQuarters, selected, onChange }: {
  ...
  return (
    <div className="mt-2.5 bg-gray-50 rounded-xl p-3 space-y-2">
  ...
  return (
    <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${event.totalQuarters}, 1fr)` }}>
      {all.map(q => {
        ...
        return (
          <div key={q} className="text-center bg-gray-50 rounded-xl py-2.5">
      ...
      <div className="flex justify-between items-center">
        ...
        {role === 'admin' && (
          <button onClick={() => setShowCreate(!showCreate)}
            className={`font-bold px-4 py-2 rounded-xl transition text-sm ${showCreate ? 'bg-gray-100 text-gray-600' : 'bg-green-600 hover:bg-green-700 text-white shadow-sm'}`}>
            {showCreate ? '취소' : '+ 일정'}
          </button>
        )}
      </div>
            ...
            <div className="flex gap-2">
              {[2, 3, 4, 5, 6].map(n => (
                <button key={n} type="button" onClick={() => setTotalQuarters(n)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition ${totalQuarters === n ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>
                  {n}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-1.5 mb-2.5">
              {([['none', '없음'], ['weekly', '매주'], ['biweekly', '격주'], ['monthly', '매월']] as const).map(([v, label]) => (
                <button key={v} type="button" onClick={() => setRepeatMode(v)}
                  className={`flex-1 py-2 rounded-xl text-xs font-bold transition ${repeatMode === v ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>
                  {label}
                </button>
              ))}
            </div>
                ...
                <div className="flex gap-1.5">
                  {[2, 3, 4, 6, 8, 12].map(n => (
                    <button key={n} type="button" onClick={() => setRepeatCount(n)}
                      className={`w-9 h-9 rounded-xl text-xs font-bold transition ${repeatCount === n ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-500'}`}>
                      {n}
                    </button>
                  ))}
                </div>
          ...
          return (
            <div key={event.id} className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                <div className="flex gap-3.5 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">
                          ...
                          <span className="ml-1.5 bg-gray-100 text-gray-500 font-bold px-1.5 py-0.5 rounded-md text-[10px]">{event.totalQuarters}Q</span>
                        </p>
                      </div>
                      {role === 'admin' && (
                    ...
                    <div className="flex gap-1.5 mt-2.5">
                      ...
                      {unvoted.length > 0 && (
                        <span className="bg-gray-100 text-gray-400 font-bold text-xs px-2.5 py-1 rounded-lg">미투표 {unvoted.length}</span>
                      )}
                    </div>
                  </div>
                </div>
              ...
              <button onClick={() => setExpandedId(isExpanded ? null : event.id)}
                className="w-full text-xs text-gray-400 hover:text-gray-600 py-3 border-t border-gray-100 transition bg-gray-50/50 font-medium">
                ...
                <div className="px-4 pb-4 pt-2 space-y-0.5">
                  {players.length === 0 ? (
                    ...
                    [...players].sort((a, b) => a.name.localeCompare(b.name)).map((p) => {
                      ...
                      return (
                        <div key={p.id} className="py-2.5 border-b border-gray-100 last:border-0">
                  ...
                  {/* 팀 나누기 */}
                  {attending.length >= 2 && (
                    <div className="pt-3 border-t border-gray-200 space-y-3 mt-1">
                      <div className="flex gap-2">
                        <input className="flex-1 bg-gray-50 border-0 rounded-xl px-3 py-2 text-xs text-gray-900 focus:outline-none focus:ring-1 focus:ring-green-400"
                  ...
                  {role === 'admin' && (
                    <div className="flex gap-2 mt-3 pt-1">
                      ...
                      <button onClick={() => closeEvent(event.id)}
                        className="flex-1 text-xs text-gray-400 hover:text-gray-600 py-2.5 bg-gray-50 rounded-xl transition font-medium">
                        투표 마감
                      </button>
                    </div>
                  )}
